...
let AppView () =
    ...
    let loginError, setLoginError = React.useState(None)

    let handleLogin = React.useCallback(fun (username: string) (password: string) ->
        promise {
            setIsLoggingIn(true)
            setLoginError(None)
            try
                let! result = verifyCredentials username password
                match result with
                | Ok () ->
                    // Credentials verified, store and authenticate
                    sessionStorage.setItem("username", username)
                    sessionStorage.setItem("password", password)
                    // Force a page refresh to re-init with authenticated state
                    Browser.Dom.window.location.reload()
                | Error err ->
                    setLoginError(Some err)
                    setIsLoggingIn(false)
            with ex ->
                setLoginError(Some $"Error: {ex.Message}")
                setIsLoggingIn(false)
        } |> ignore
    , [||])
                                        ...
                                        Html.form [
                                            ...
                                            prop.onSubmit (fun e ->
                                                ...
                                                if state.Username <> "" && state.Password <> "" then
                                                    ...
                                                    setLoginError(Some "Please enter username and password")
                                            )
                                            prop.children [
                                                // Error alert
                                                ...
                                                | Some error ->
                                                    Daisy.alert [
                                                        ...
                                                        prop.className "mb-2"
                                                        prop.children [
                                                            Html.i [ prop.className "fa-solid fa-circle-exclamation" ]
